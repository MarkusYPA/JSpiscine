...
function getURL(dataSet) {
    // first http:// or https://, then something, end at whitespace
    let re = /(http:\/\/|https:\/\/)\S+(?=\s)/g;

    let matches = String(dataSet).match(re);
    if (matches == null) {
        return [];
    }
    return matches;
...
function greedyQuery(dataSet) {
    // at least 3 query parameters:
    // ?, (some non-white spaces), &, (some non-white spaces), &
    let re = /\?\S+\&\S+\&/;

    let greedies = [];
    for (let e of getURL(dataSet)) {
        if (re.test(e)) {
            greedies.push(e);
        }
    }
...
function notSoGreedy(dataSet) {
    ...
    let moderates = [];
    for (let e of getURL(dataSet)) {
        if (re2.test(e) && !re4.test(e)) {  // at least 2 but not 4
            moderates.push(e);
        }
    }
    return moderates;
...
/* console.log(arraysEqual(getURL(dataSet), $getURL))
console.log(arraysEqual(greedyQuery(dataSet), $greedyQuery))
console.log(arraysEqual(notSoGreedy(dataSet), $notSoGreedy)) */