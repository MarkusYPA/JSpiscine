...
console.log(JSON.stringify(
    filterKeys(cart, (k) => k.length <= 6)) == JSON.stringify(
        filtered)
)
console.log(JSON.stringify(
    filterKeys(cart, (k) => /onion/.test(k))) == JSON.stringify(
        { onion: 200 })
)

// map keys
console.log(JSON.stringify(
    mapKeys(cart, (k) => `✔️${k}`)) == JSON.stringify(
        mapped)
)
console.log(JSON.stringify(mapKeys(filterKeys(cart, k => k === 'onion'), ((k) => (k = 'orange')))) == JSON.stringify({ orange: 200 }))
console.log(JSON.stringify(mapKeys(filterKeys(nutritionDB, k => k === 'tomato'), (k) => `${k}DB`)) == JSON.stringify(combo))
